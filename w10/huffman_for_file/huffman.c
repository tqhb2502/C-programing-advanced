#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include "huffman.h"

// kiểm tra pqueue còn 1 phần tử
int hasOneNode(Dllist list) {
    int counter = 0;

    Dllist node;
    dll_traverse(node, list) {
        counter++;
    }

    return (counter == 1) ? 1 : 0;
}

// đếm tổng số lần các kí tự (ứng với nút trên cây) xuất hiện
int countAppear(JRB treeNode, int *appear) {
    int counter = 0;

    char *tmp_string = jval_s(treeNode->val);
    for (int i = 0; i < strlen(tmp_string); i++) {
        counter += appear[tmp_string[i]];
    }

    return counter;
}

// tìm phần tử có khóa nhỏ nhất trong pqueue
Dllist pqueueGetMin(Dllist pqueue, int *appear) {
    int minAppear = INFINITIVE_VALUE;

    Dllist node, minNode;
    JRB treeNode;
    int cur;
    dll_traverse(node, pqueue) {
        treeNode = (JRB) jval_v(node->val);
        cur = countAppear(treeNode, appear);
        if (cur < minAppear) {
            minAppear = cur;
            minNode = node;
        }
    }

    return minNode;
}

// tạo cây Huffman
HuffmanTree makeHuffman(FILE *input) {
    // tạo cây
    HuffmanTree newHuffmanTree;
    newHuffmanTree.graph = createGraph();

    // đếm số lần xuất hiện của mỗi ký tự trong xâu buffer
    int appear[256];
    for (int i = 0; i <= 255; i++) {
        appear[i] = 0;
    }
    char fileChar;
    while (!feof(input)) {
        fileChar = fgetc(input);
        appear[fileChar]++;
    }

    // kí tự nào xuất hiện thì tạo nút của ký tự đó trong cây, thêm nút vào pqueue
    Dllist pqueue = new_dllist();
    int idCounter = 0;
    char tmp_string[2];
    for (int i = 0; i <= 255; i++) {
        if (appear[i] > 0) {
            tmp_string[0] = (char) i;
            tmp_string[1] = '\0';
            addVertex(newHuffmanTree.graph, idCounter, tmp_string);

            dll_append(pqueue, new_jval_v(getVertex(newHuffmanTree.graph, idCounter)));

            idCounter++;
        }
    }

    // ghép cây
    char tmp_string_2[257];
    Dllist minNode_1, minNode_2;
    JRB u, v, parent;
    while (!hasOneNode(pqueue)) {
        // lấy 2 nút có tổng số lần xuất hiện các ký tự ít nhất
        minNode_1 = pqueueGetMin(pqueue, appear);
        u = (JRB) jval_v(minNode_1->val);
        dll_delete_node(minNode_1);
        minNode_2 = pqueueGetMin(pqueue, appear);
        v = (JRB) jval_v(minNode_2->val);
        dll_delete_node(minNode_2);

        // tạo nút cha cho 2 nút trên
        strcpy(tmp_string_2, jval_s(u->val));
        strcat(tmp_string_2, jval_s(v->val));
        addVertex(newHuffmanTree.graph, idCounter, tmp_string_2);

        // tạo cạch nối cha-con
        parent = getVertex(newHuffmanTree.graph, idCounter);
        addEdge(newHuffmanTree.graph, jval_i(parent->key), jval_i(u->key), 0);
        addEdge(newHuffmanTree.graph, jval_i(parent->key), jval_i(v->key), 1);

        // thêm cha vào pqueue
        dll_append(pqueue, new_jval_v(parent));

        idCounter++;
    }

    // gán root của cây là phần tử cuối cùng còn lại trong pqueue
    newHuffmanTree.root = (JRB) jval_v(dll_first(pqueue)->val);

    return newHuffmanTree;
}

// thăm cây Huffman đã tạo ở trên để cho ra HuffmanTable
void treeTraverse(Graph g, JRB curNode, char *bitSequence, int bitSize, Coding *htable) {
    JRB node = jrb_find_int(g.edges, jval_i(curNode->key));

    if (node != NULL) {
        // nếu nút đang thăm không phải nút lá, thăm các con của nó
        JRB adj_list = (JRB) jval_v(node->val);

        jrb_traverse(node, adj_list) {
            // lấy key nút con, và trọng số cạnh nối
            int key = jval_i(node->key);
            int w = (int) jval_d(node->val);
            
            // nếu trọng số là 1, thêm bit 1, ngược lại, thêm bit 0
            if (w == 1) {
                bitSequence[bitSize] = '1';
            } else {
                bitSequence[bitSize] = '0';
            }
            bitSize++;

            // thăm nút con
            JRB nextNode = getVertex(g, key);
            treeTraverse(g, nextNode, bitSequence, bitSize, htable);

            // thăm xong nút con, xóa bỏ bit ở cạnh nối cha-con đã thêm ở trên
            bitSize--;
        }
    } else {
        // nếu nút đang thăm là nút lá, thêm ký tự của nút này vào HuffmanTable
        char *tmp_string = jval_s(curNode->val);
        char ch = tmp_string[0];
        htable[ch].size = bitSize;

        for (int i = 0; i < bitSize; i++) {
            htable[ch].bits[i] = bitSequence[i];
        }
        htable[ch].bits[bitSize] = '\0';
    }
    return;
}

// tạo HuffmanTable(bảng chuyển từ ký tự sang bit)
void createHuffmanTable(HuffmanTree htree, Coding *htable) {
    char bitSequence[9];
    treeTraverse(htree.graph, htree.root, bitSequence, 0, htable);
    return;
}

// tìm số lớn hơn
int getMax(int a, int b) {
    return (a > b) ? a : b;
}

// tìm chiều cao của cây
int getTreeHeight(Graph g, JRB curNode) {
    JRB node = jrb_find_int(g.edges, jval_i(curNode->key));

    if (node != NULL) {
        JRB adj_list = (JRB) jval_v(node->val);
        
        int max = -1;
        jrb_traverse(node, adj_list) {
            int key = jval_i(node->key);
            JRB nextNode = getVertex(g, key);
            max = getMax(max, getTreeHeight(g, nextNode));
        }

        return max + 1;
    } else {
        return 1;
    }
}

// chuyển HuffmanTree thành mảng HuffmanTreeArray
HuffmanTreeArray tree2array(HuffmanTree htree) {
    // tạo mảng
    HuffmanTreeArray htarray;

    int treeHeight = getTreeHeight(htree.graph, htree.root);
    int power = 1;
    for (int i = 1; i <= treeHeight; i++) {
        power = power * 2;
    }

    htarray.size = power - 1;
    htarray.node = (int *) malloc(htarray.size * sizeof(int));
    for (int i = 0; i < htarray.size; i++) {
        htarray.node[i] = 257;
    }

    // điền các giá trị vào mảng (dùng BFS)
    int *pos = (int *) malloc(htarray.size * sizeof(int)); // mảng lưu vị trí của nút trong HTArray

    Dllist queue = new_dllist();
    pos[jval_i(htree.root->key)] = 0; // cho nút gốc vào vị trí 0
    dll_append(queue, htree.root->key);

    Dllist queueNode;
    int u, v, w;
    while (!dll_empty(queue)) {
        queueNode = dll_last(queue);
        u = jval_i(queueNode->val);
        dll_delete_node(queueNode);

        JRB node = jrb_find_int(htree.graph.edges, u);

        if (node != NULL) {
            JRB adj_list = (JRB) jval_v(node->val);

            jrb_traverse(node, adj_list) {
                v = jval_i(node->key);
                w = (int) jval_d(node->val);

                if (w == 0) {
                    pos[v] = pos[u] * 2 + 1; // con trái
                } else {
                    pos[v] = pos[u] * 2 + 2; // con phải
                }

                dll_append(queue, new_jval_i(v));
            }

            htarray.node[pos[u]] = -1; // có con -> không phải nút lá -> -1
        } else {
            JRB treeNode = getVertex(htree.graph, u);
            char *tmp_string = jval_s(treeNode->val);

            htarray.node[pos[u]] = tmp_string[0]; // nút lá
        }
    }

    return htarray;
}

// nén ký tự ch
void addHuffmanChar(char ch, Coding *htable, char *huffman, int *nbit) {
    if (0 <= ch && ch <= 255) {
        for (int i = 0; i < htable[ch].size; i++) {
            huffman[*nbit] = htable[ch].bits[i];
            (*nbit)++;
        }
    }
    return;
}

/*// nén xâu buffer
void compress(char *buffer, int size, char *huffman, int *nbit, Coding *htable) {
    for (int i = 0; i < size; i++) {
        addHuffmanChar(buffer[i], htable, huffman, nbit);
    }
    huffman[*nbit] = '\0';
    return;
}*/

void compressFile(FILE *input, FILE *output) {
    // tạo cây Huffman
    HuffmanTree htree = makeHuffman(input);

    // tạo bảng Huffman
    Coding htable[256];
    for (int i = 0; i <= 255; i++) {
        htable[i].size = 0;
    }
    createHuffmanTable(htree, htable);

    // chuyển cây Huffman thành mảng
    HuffmanTreeArray htarray = tree2array(htree);

    // ghi prefix code của file sau khi nén
    fputs("HM", output);

    // ghi mảng biểu diễn HuffmanTree
    fputs("@_", output); // ký hiệu @_ ngăn cách các thuộc tính khác nhau của file sau khi nén
    fprintf(output, "%d", htarray.size);

    fputs("@_", output);
    for (int i = 0; i < htarray.size; i++) {
        if (htarray.node[i] == 257) {
            fprintf(output, "@#"); // @# biểu diễn phần tử trống
        } else if (htarray.node[i] == -1) {
            fprintf(output, "@$"); // @$ biểu diễn phần tử -1
        } else {
            fprintf(output, "%c", htarray.node[i]);
        }
    }

    // nén file input
    char huffman[8000000];
    int nbit = 0;

    rewind(input);
    while (!feof(input)) {
        addHuffmanChar(fgetc(input), htable, huffman, &nbit);
    }
    huffman[nbit] = '\0';

    fputs("@_", output);
    fprintf(output, "%d", nbit);

    fputs("@_", output);
    fprintf(output, "%s", huffman);
    
    return;
}

void decompressFile(FILE *input, FILE *output) {
    // kiểm tra xem có phải file nén không
    char checkCompressed[3];
    checkCompressed[0] = fgetc(input);
    checkCompressed[1] = fgetc(input);
    checkCompressed[2] = '\0';

    if (strcmp(checkCompressed, "HM") != 0) {
        printf("Invalid compressed file!\n");
        exit(0);
    }

    // tạo mảng biểu diễn HuffmanTree
    HuffmanTreeArray htarray;

    fgetc(input); //
    fgetc(input); // lấy phần tử ngăn cách @_

    // lấy kích thước của mảng
    htarray.size = 0;
    char ch;
    while ((ch = fgetc(input)) != '@') {
        htarray.size = htarray.size * 10 + (ch - 48);
    }
    fgetc(input);

    // cấp phát bộ nhớ cho mảng
    htarray.node = (int *) malloc(htarray.size * sizeof(int));

    // điền các phần tử vào mảng
    int id = 0;
    int curChar, nextChar = fgetc(input);
    while (1) {
        curChar = nextChar;
        nextChar = fgetc(input);

        if (curChar == '@') {
            if (nextChar == '_') {
                break;
            } else if (nextChar == '#') {
                htarray.node[id] = 257;
                nextChar = fgetc(input);
            } else if (nextChar == '$') {
                htarray.node[id] = -1;
                nextChar = fgetc(input);
            } else {
                htarray.node[id] = curChar;
            }
        } else {
            htarray.node[id] = curChar;
        }

        id++;
    }

    // lấy số lượng bit của chuỗi sau khi nén
    int nbit = 0;
    while ((ch = fgetc(input)) != '@') {
        nbit = nbit * 10 + (ch - 48);
    }
    fgetc(input);

    // giải nén
    int curID = 0;
    while (!feof(input)) {
        ch = fgetc(input);

        if (ch == '0' || ch == '1') {
            curID = curID * 2 + (ch - 48) + 1;

            if (0 <= htarray.node[curID] && htarray.node[curID] <= 255) {
                fprintf(output, "%c", htarray.node[curID]);
                curID = 0;
            }
        }
    }
}